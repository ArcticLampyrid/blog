...
然后还有 [UEXECUTER](../uexecuter.md)，一个可爱的函数调用器，可通过串口通讯调用单片机上的函数，主要是为了方便调试用的。同时也作为社团留任的能力展示项目。
...
[njupt-wifi-login](../yet-another-wifi-login-solver-for-njupter.md)：南邮校园网登录器。今年暑假南邮的校园验证系统进行了一次大更新（终于支持 TLS 了），于是随之更新了一下这个登录器。随后为了方便使用，又在除夕前加上了多语言支持、作为 Windows 服务启动、Linux 兼容等功能。说实话，这是我第一次写 Windows Service，也是我第一次写 Rust 的 GUI 项目，更是第一次使用 Linux-specific 的一些特性。虽然这个程序的定位是 Utility（实用工具），但却意外地打破了许多技术边界，真是件有趣的事情。
